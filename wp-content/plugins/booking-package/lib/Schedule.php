<?php
    ...
    class booking_package_schedule {
    		...
    		if (isset($_POST['deletePublishedSchedules_from_month']) && isset($_POST['deletePublishedSchedules_from_day']) && isset($_POST['deletePublishedSchedules_from_year'])) {
				...
				if ($_POST['delete_action'] == 'delete') {
					...
					if ($_POST['deletionType'] == 'perfect') {
						...
						$sql = $wpdb->prepare(
							"UPDATE `".$table_name."` SET `status` = 'deleted' WHERE `capacity`= `remainder` AND `accountKey` = %d;", 
							array($accountKey)
						);
						...
						if ($_POST['period'] == 'period_after') {
							...
							$sql = $wpdb->prepare(
								"UPDATE `".$table_name."` SET `status` = 'deleted' WHERE `capacity`= `remainder` AND `accountKey` = %d AND `unixTime` > %d;", 
								array($accountKey, intval($unixTime_from))
							);
						...
						if ($_POST['period'] == 'period_within') {
							...
							$sql = $wpdb->prepare(
								"UPDATE `".$table_name."` SET `status` = 'deleted' WHERE `capacity`= `remainder` AND `accountKey` = %d AND (`unixTime` > %d AND `unixTime` < %d);", 
								array($accountKey, intval($unixTime_from), intval($unixTime_to))
							);
    	...
    	public function getSelectedServices($calendarAccount, $selectedServices, $guests, $targetOptions, $coupon = array(), $applicantCount = 1) {
    		...
    		$time = 0;
    		$cost = 0;
    		$hasKeys = array("key" => "int", "accountKey" => "int", "name" => "string", "time" => "int", "cost" => "int", "cost_1" => "int", "cost_2" => "int", "cost_3" => "int", "cost_4" => "int", "cost_5" => "int", "cost_6" => "int", "active" => "string", "options" => "object", "selectedOptionsList" => "object", "service" => "int", "selected" => "int");
