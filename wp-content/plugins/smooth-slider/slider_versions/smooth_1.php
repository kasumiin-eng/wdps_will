...
function carousel_posts_on_slider($max_posts, $offset=0, $slider_id = '1',$out_echo = '1', $set='', $data=array() ) {
	...
	$posts = $wpdb->get_results("SELECT b.* FROM 
		...
		ON a.post_id = b.ID 
		WHERE (b.post_status = 'publish' OR (b.post_type='attachment' AND b.post_status = 'inherit')) AND a.slider_id = '$slider_id' AND (a.expiry IS NULL OR a.expiry='0000-00-00' OR DATE(a.expiry) >= CURDATE() ) ORDER BY ".$orderby." LIMIT $offset, $max_posts", OBJECT);
	}
