...
global $smooth_slider;
	...
	if(isset($_POST['sldr-verify']) and current_user_can( $smooth_slider['user_level'] ) ) {
		...
		if(isset($_POST['display_slider']) and isset($_POST['display_slider_name'])){
		  $slider_id = (int) $_POST['display_slider_name'];
		}
		...
		$post_slider_thumbnail=isset($_POST['sslider_thumbnail'])?sanitize_text_field($_POST['sslider_thumbnail']):'';
		...
		$link=isset($_POST['sslider_link'])?sanitize_text_field($_POST['sslider_link']):'';
		...
		$post_sslider_nolink = isset($_POST['sslider_nolink'])?sanitize_text_field($_POST['sslider_nolink'] ):'';
		...
		$post_disable_image = isset($_POST['disable_image'])?sanitize_text_field($_POST['disable_image'] ):'';
		...
		$post_slider_style=isset($_POST['_smooth_slider_style'])?sanitize_text_field($_POST['_smooth_slider_style']):'';
...
function remove_from_slider($post_id) {
	if(isset($_POST['sldr-verify'])) {
		...
		$display_slider = isset($_POST['display_slider'])?sanitize_text_field( $_POST['display_slider'] ):'';
		$table_name = $table_prefix.SLIDER_POST_META;
...
function add_to_slider_checkbox() {
	...
	if( function_exists('icl_plugin_action_links') ) {
		if( isset($_GET['source_lang']) && isset($_GET['trid']) ) {
			global $wpdb, $table_prefix;
			$id = $wpdb->get_var( "SELECT element_id FROM {$wpdb->prefix}icl_translations WHERE trid=".(int) $_GET['trid']." AND language_code='".sanitize_text_field($_GET['source_lang'])."'" );			
			$table_name = $table_prefix.SLIDER_TABLE;
